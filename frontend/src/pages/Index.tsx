          ...
          <div className="relative container mx-auto px-4 py-24 md:py-32">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                ...
                <a href="#features">
                  <Button id="learn-more-btn" size="lg" variant="outline" className="hover:-translate-y-1 hover:bg-muted/50 transition-all duration-300 border-border bg-background">
                    Learn More
                  </Button>
                </a>
              </div>
              ...
              <div className="mt-16 relative">
                ...
                <Card className="relative border-border bg-card/80 backdrop-blur-sm shadow-2xl">
                  <CardContent className="p-0">
                    <div className="flex items-center gap-2 px-4 py-3 border-b border-border bg-muted/30">
                      <div className="w-3 h-3 rounded-full bg-destructive/60" />
                      <div className="w-3 h-3 rounded-full bg-yellow-500/60" />
                      <div className="w-3 h-3 rounded-full bg-success/60" />
                      <span className="ml-2 text-xs text-muted-foreground font-medium">Talk2SQL.ai</span>
                    </div>
                    <div className="p-6 text-left">
                      ...
                      <div>
                        ...
                        <pre className="mt-2 p-4 rounded-lg bg-code-bg text-sm font-mono overflow-x-auto border border-border/50">
...
GROUP BY u.id;`}
        ...
        {/* Features Section */}
        <section id="features" className="py-24 bg-muted/30 transition-colors duration-300">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {features.map((feature, index) => {
                ...
                return (
                  <Card
                    key={index}
                    className="h-full border-border bg-card hover:bg-muted/50 transition-all duration-300 group cursor-default"
                  >
                    <CardContent className="p-6">
                      <div className="w-12 h-12 rounded-lg gradient-primary flex items-center justify-center mb-4 glow-primary-sm group-hover:scale-110 transition-transform">
                ...
                <Card className="relative border-border bg-card p-8">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {['🐬 MySQL', '🐘 PostgreSQL', '📦 SQLite'].map((db, index) => (
                      <div
                        key={index}
                        className="p-4 rounded-lg bg-muted/50 text-center text-sm font-medium border border-border/50"
                      >
                        {db}
                      </div>
                    ))}
